	...
	public class randomPather : MonoBehaviour
	{
		...
		private async void getPath()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				...
				path = await AStarPathfinding.GeneratePath(presentX, presentY, randomX, randomY, walkableMap);
		...
		void Start()
		{
			(int, int) presentCord = DemoGrid.Instance.worldSpaceToCordinate(transform.position);
			presentX = presentCord.Item1;
			presentY = presentCord.Item2;
		}
		...
		void Update()
		{
			if (path.Length != 0)
			{
				if (Vector3.Distance(target, transform.position) < 0.1f)
				{
					...
					{
						presentX = path[path_index].Item1;
						presentY = path[path_index].Item2;
						path_index++;
					}
