	...
	public class AStarBoolMap
	{
		...
		private static (bool, (int, int))[] getNeighbours(int xCordinate, int yCordinate, bool[,] walkableMap, bool walkableDiagonals = false)
		{
			...
			for (int y = yStart; y <= yEnd; y++)
			{
				for (int x = xStart; x <= xEnd; x++)
				{
					...
					!walkableMap[y, xCordinate] && !walkableMap[yCordinate, x]) // If the node is not reachable from the present node
					{
						continue;
					}
		...
		public static (int, int)[] GeneratePath(int startX, int startY, int goalX, int goalY, bool[,] walkableMap, bool manhattanHeuristic = true, bool walkableDiagonals = false)
		{
			...
			while (openSet.Count > 0)
			{
				...
				// Remove the present node from the open set
				openSet.Dequeue();

				// Get the neighbours of the present node
				(bool, (int, int))[] neighbours = getNeighbours(presentX, presentY, walkableMap, walkableDiagonals);
				...
				for (int i = 0; i < neighbours.Length; i++)
				{
					...
					float tentativeGCost = gCostMap[presentY, presentX] + dist;
