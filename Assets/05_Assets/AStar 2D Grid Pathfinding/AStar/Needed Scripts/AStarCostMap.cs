...
namespace AStar.Algolgorithms
{
	public class AStarCostMap
	{
		...
		private static (int, int)[] backtracePath((int, int)[,] parentMap, int goalX, int goalY)
		{
			...
			(int, int) present = (goalX, goalY);

			while (present != (-1, -1))
			{
				path.Add(present);
				present = parentMap[present.Item2, present.Item1];
			}
		...
		private static (float, (int, int))[] getNeighbours(int xCordinate, int yCordinate, float[,] costMap, bool walkableDiagonals = false)
		{
			...
			for (int y = yStart; y <= yEnd; y++)
			{
				for (int x = xStart; x <= xEnd; x++)
				{
					...
					costMap[y, xCordinate] == -1f && costMap[yCordinate, x] == -1f) // If the node is not reachable from the present node
					{
						continue;
					}
		...
		public static (int, int)[] GeneratePath(int startX, int startY, int goalX, int goalY, float[,] costMap, bool manhattanHeuristic = true, bool walkableDiagonals = false)
		{
			...
			while (openSet.Count > 0) // While there are still nodes in the open set
			{
				...
				(int, int) present = openSet.First;
				int presentX = present.Item1;
				int presentY = present.Item2;

				// If we have reached the goal node, backtrack to get the path
				if (present.Item1 == goalX && present.Item2 == goalY)
				{
					return backtracePath(parentMap, goalX, goalY);
				}

				openSet.Dequeue();
				(float, (int, int))[] neighbours = getNeighbours(presentX, presentY, costMap, walkableDiagonals);

				// Loop through all the neighbours of the present node
				for (int i = 0; i < neighbours.Length; i++)
				{
					...
					// Calculate the tentative g-cost of the neighbour using the distance between the present node and the neighbour, and the cost of the neighbour
					float dist = presentX - neighbourX == 0 || presentY - neighbourY == 0 ? 1 : sqrt2;
					float tentativeGCost = gCostMap[presentY, presentX] + dist * neighbour.Item1;

					// If the neighbour has not been visited yet, or the tentative g cost is lower than its present g cost,
					...
					{
						parentMap[neighbourY, neighbourX] = present;
